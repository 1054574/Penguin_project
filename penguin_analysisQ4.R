...
#Now, we will save the clean data in a separate folder called "data_clean" in a .csv file.
write.csv(penguins_clean_now, "data_clean/penguins_clean.csv")
...
#Plot histograms and qqplots to check the normallity of the two groups and for the presence of outliers.
#Histogram
...
#Looking at the histograms, they appear to be normally distributed for both groups and there are no major outliers in the data sets.  
...
levene_test
#The p value is greater than 0.05, which indicates that we fail to reject the null hypothesis, which means that the variances of the two gorups are not significantly different from each other. 

#Now lets run the actual independent sample t-test 
